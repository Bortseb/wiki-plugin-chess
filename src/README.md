...
| ---------------------- | ---- | ----------------------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `chess.js`             | 5060 | `chess.js`                          | Wiki shell           | iframe/popup embed, journal save gateway, **all cross-wiki fetch**, `shellAppMessageHandlers`                                                             |
| `chess-app.js`         | 4360 | `chess-app.js`                      | iframe / popup / PWA | UI orchestrator — boot, view router, auth/PWA session, journal gateway, init hosts (`import * as` mode modules)                                           |
| `game.js`              | 4050 | _(inlined in chess-app)_            | App                  | GAME mode — console, seats, start-game modal, player bars, challenges, resign, comments                                                                   |
| `chess-core.js`        | 5130 | _(inlined in bundles)_              | Shared               | PGN/FEN/keywords, `MSG`, game-sync policy (pure), realtime presence helpers, session FSM, journal planning; namespaces `ChessRules` / `Paste` / `Journal` |
| `federation.js`        | 6220 | _(inlined in bundles)_              | Shared               | Glicko-2, federation crawl/consensus, site-survey deferred orchestration, charm/gossip page operations, browser site client                               |
| `glicko-worker.js`     | 50   | `glicko-worker.js`                  | Wiki shell Worker    | Off-main-thread Glicko timeline replay                                                                                                                    |
| `survey.js`            | 3250 | _(inlined in chess-app)_            | App                  | IndexedDB ratings, site-survey / open-challenge UI, shared `#leaderboard` browse shell                                                                    |
| `leaderboard.js`       | 1240 | _(inlined in chess-app)_            | App                  | Federated leaderboard UI (gate, hop dials, ranking table) — registers into survey                                                                         |
| `puzzle.js`            | 3610 | _(inlined in chess-app)_            | App                  | Puzzle solving + authoring UI, bulk CSV parse (`parsePuzzlesCsv`), on-device puzzle DB                                                                    |
| `board-layout.js`      | 3250 | _(inlined in chess.js + chess-app)_ | Shell + app          | Board fit, embed/PWA sizing, PWA protocol + page chrome, paste, `shellTransport` / `PWA` / `Transport`                                                    |
| `choose-menu.js`       | 620  | _(inlined in chess-app)_            | App                  | CHOOSE menu / game-lifecycle navigation                                                                                                                   |
| `position.js`          | 250  | _(inlined in chess-app)_            | App                  | POSITION mode — FEN editor board, save/flip, entry from CHOOSE                                                                                            |
| `realtime.js`          | 1320 | _(inlined in chess-app)_            | App                  | Game sync coordinator, remote poll, WebRTC, `applySyncedPosition`                                                                                         |
| `modals.js`            | 1680 | _(inlined in bundles)_              | Shell + app          | Shared modal/dialog library                                                                                                                               |
| `cm-modules-bundle.js` | 1420 | `cm-modules-bundle.js`              | App (runtime import) | chess-console + cm-\* wiki glue                                                                                                                           |
...
| `server/pwa-bridge.js`     | HTTP journal + federation bridge (`GET /session`, save, crawl routes)                                                           |
...
- Cross-wiki fetch: `createBrowserWikiSiteClient(wiki)` in `federation.js` (shell); `createPwaBridgeSiteClient` in `server/pwa-bridge.js` (installed PWA).
